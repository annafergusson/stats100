...
/*    
var target = document.getElementById('tutorial-exercise-glue-eg-output')
var observer = new MutationObserver(function(mutations) {
  var old = $('#tutorial-exercise-glue-eg-output > pre > code').text();   
  $('#tutorial-exercise-glue-eg-output > pre > code').html(old.replace("FUCK","F**K"));
});
...
word <- sample(letters, size = 4, replace = TRUE) %>%
  str_flatten()
...
word <- sample(letters, size = 3) %>% str_flatten()
...
```{r placement, exercise=TRUE, exercise.lines=16}
adjectives <- c("MAJESTIC", "CHAOTIC", "STUNNING", "FIERCE", "UNIQUE", "POWERFUL", "WHOLESOME", "FANCY")
