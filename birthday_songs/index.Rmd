...
getMusicData <- function(chart_id){
  week_url <- glue("https://nztop40.co.nz/chart/singles?chart={chart_id}")
    ...
    track_apple_info <- map_df(1 : nrow(nztop40), function(i){
      ...
      info <- getTrackDetails(search_artist, search_title)  %>%
        select(trackId, artistName, collectionName, trackName, releaseDate, trackTimeMillis, primaryGenreName) %>%
        mutate(single_artist = search_artist, single_title = search_title)
    ...
    nztop40 %>%
      ...
      mutate(single_last_week = ifelse(single_last_week == "new",
                                       "",
                                       single_last_week),
...
getTrackDetails <- function(artist, track){
  ...
  {
  return(artist_song %>%
    filter(str_detect(trackName %>% str_to_lower(), 
                      track %>% str_to_lower() %>% str_replace_all("\\(.*\\)", "") %>% str_squish())) %>%
    head(1))
  }
}
